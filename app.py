...
def compute_summary(filters=None):
    ...
    for order_id, order_rows in groups.items():
        ...
        packing_total = sum(max(float(r.get("quantity") or 0), 0) * max(float(r.get("packing_per_unit") or 0), 0) for r in basis_rows)
        ...
        for r in basis_rows:
            ...
            packing = max(qty, 0) * float(row.get("packing_per_unit") or 0) if cost_burned else 0
