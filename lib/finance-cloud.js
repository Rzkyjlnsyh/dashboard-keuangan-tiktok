...
async function computeDataQuality(filters = { preset: "thisMonth", month: "", store: "all" }) {
  ...
  // 1. SKU TANPA HPP
  const skuCostMap = new Map(allSkuCosts.map(r => `${String(r.store_name||"").toLowerCase()}|${String(r.sku||"").toLowerCase()}`));
  const skuGlobalMap = new Map(allSkuCosts.filter(r => (r.store_name||"").toLowerCase()==="global").map(r=>[String(r.sku||"").toLowerCase(),r]));
