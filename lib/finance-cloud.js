...
function mapOrderRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    ...
    return {
      ...
      cancel_reason: String(field(row, "Cancel Reason", "Cancelation/Return Type")).trim(),
...
function mapSettlementRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    const orderId = String(field(row, "Order ID", "Nomor Pesanan (Di Marketplace)") || "").trim();
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  // Fix store-level profit to match overridden totals
  for (const [storeName, storeData] of stores.entries()) {
    if (storeName === (filters.store !== 'all' ? filters.store : DEFAULT_STORE)) {
