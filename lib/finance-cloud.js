...
function isReturnStatus(status, cancelReason) {
  const token = columnToken(status);
  // Only explicit status-based return (e.g., "Retur", "Dikembalikan", "Returned")
  // Cancel_reason "Return/Refund" on completed orders = partial return, not order-level return
  if (token.includes("retur") || ["returned", "return", "returnrefund", "refundreturn", "pengembalian"].includes(token)) return true;
  // Also detect return via cancel_reason for cancelled orders where the reason explicitly says return
  const crToken = columnToken(cancelReason || "");
  if (crToken === "returnrefund" && token.includes("batal")) return true;
  return false;
}
