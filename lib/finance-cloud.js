...
function isReturnStatus(status, cancelReason) {
  const token = columnToken(status);
  // Explicit retur statuses
  if (token.includes("retur") || ["returned", "return", "returnrefund", "refundreturn", "pengembalian"].includes(token)) return true;
  // Return/Refund stored in cancel_reason for completed orders with partial return
  const crToken = columnToken(cancelReason || "");
  if (crToken.includes("returnrefund") || crToken === "returnrefund") return true;
  return false;
}
...
function dateRangeFromFilters(filters) {
  ...
  if (filters.preset === "month" && filters.month) {
    ...
    // End date = first day of NEXT month (for < comparison)
...
async function importIncomeRows({ storeName, filename, rows, run }) {
  ...
  // Store raw income — use simple INSERT to avoid constraint issues
  const rawInserted = [];
  for (const ri of rawIncomeRows) {
    try {
      // Check if exists
      const ex = await pg.pgQuery(
        `SELECT 1 FROM finance_income_raw WHERE store_name=$1 AND order_id=$2 AND transaction_type=$3`,
        [ri.store_name, ri.order_id, ri.transaction_type]
      );
      if (ex.rows && ex.rows.length > 0) continue;
      await pg.pgQuery(
        `INSERT INTO finance_income_raw(store_name,transaction_type,order_id,order_created_time,settlement_amount,total_fees,refund_amount,adjustment_amount,imported_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "ads") {
    ...
    for (const row of cleanRows) {
      ...
      const desc = String(field(row, "Description") || "").toLowerCase();
      // Only Add balance, Success, NOT GMV Pay
      ...
      const spendDate = rawDate instanceof Date && !isNaN(rawDate) ? rawDate.toISOString().slice(0,10).replace(/\//g,'-') : String(rawDate).replace(/\//g,'-').slice(0,10);
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    const operationPackageCount = Math.max(new Set(operationRows.map(row => {
      const tid = String(row.tracking_id || "").trim();
      const pid = String(row.package_id || "").trim();
    ...
    // Track missing costs before calculations (don't filter out rows)
    const statusToken = columnToken(first.status || "");
    // isCompleted based on STATUS, not on income settlement (to allow accrual estimation)
    const isCompleted = ["selesai", "completed", "delivered", "terkirim"].includes(statusToken);
    ...
    const isShipped = statusToken.includes("dikirim") || statusToken.includes("shipped") ||
      statusToken.includes("intransit") || statusToken.includes("delivery");
    ...
    const cancelReason = String(first.cancel_reason || "").trim();
    const isCancelValidReason = columnToken(cancelReason).includes("pengirimanpaketgagal") || columnToken(cancelReason).includes("pakethilang") || columnToken(cancelReason).includes("gagalkirim");
  ...
  // === DIRECT DB QUERY for settlement (bypasses fetchAll issues) ===
  if (calcMode === "settlement" && startDate && endDate) {
    try {
      const endNext = new Date(new Date(endDate+'T00:00:00Z').getTime() + 86400000).toISOString().slice(0,10);
      const store = filters.store !== 'all' ? filters.store : 'custombase';
      
      // Settlement + fees
      const sq = await pg.pgQuery(
        `SELECT SUM(settlement_amount)::numeric as s, SUM(total_fees)::numeric as f FROM finance_income_raw WHERE store_name=$1 AND transaction_type=$2 AND order_created_time >= $3 AND order_created_time < $4`,
        [store, 'Pesanan', startDate, endNext]
      );
      const sr = sq.rows?.[0] || {};
      totals.settlement = Math.round(Number(sr.s||0));
      totals.platformFee = Math.round(Math.abs(Number(sr.f||0)));
      totals.platformFeeFinal = totals.platformFee;
      totals.platformFeeEstimated = 0;
      
      // GMV Ads
      const gq = await pg.pgQuery(
        `SELECT SUM(ABS(settlement_amount))::numeric as s FROM finance_income_raw WHERE store_name=$1 AND transaction_type=$2 AND order_created_time >= $3 AND order_created_time < $4`,
        [store, 'Pembayaran GMV untuk Iklan TikTok', startDate, endNext]
      );
      totals.adSpendSettlement = Math.round(Number(gq.rows?.[0]?.s||0));
      totals.settlementAdSpend = totals.adSpendSettlement;
      
      // Penggantian
      const pq = await pg.pgQuery(
        `SELECT SUM(settlement_amount)::numeric as s FROM finance_income_raw WHERE store_name=$1 AND transaction_type LIKE $2 AND order_created_time >= $3 AND order_created_time < $4`,
        [store, '%Penggantian%', startDate, endNext]
      );
      totals.adjustmentAmount = Math.round(Number(pq.rows?.[0]?.s||0));
      
      // Refund
      const rq = await pg.pgQuery(
        `SELECT SUM(ABS(refund_amount))::numeric as r FROM finance_income_raw WHERE store_name=$1 AND transaction_type=$2 AND order_created_time >= $3 AND order_created_time < $4`,
        [store, 'Pesanan', startDate, endNext]
      );
      totals.refund = Math.round(Number(rq.rows?.[0]?.r||0));
    ...
    } catch(e) { console.error('Direct settle query error:', e.message); }
  }
