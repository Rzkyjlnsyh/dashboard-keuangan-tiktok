...
async function computeSummary(filters = { preset: "all", month: "", store: "all" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    const finalPlatformFeeTotal = hasIncome ? rawPlatformFeeTotal : 0;
    const estimatedPlatformFeeTotal = hasIncome ? 0 : rawPlatformFeeTotal;
    const platformFeeTotal = hasIncome ? finalPlatformFeeTotal : estimatedPlatformFeeTotal;
    ...
    if (excluded) {
      ...
      const settlementGap = hasIncome && settlementTotal
