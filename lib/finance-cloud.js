  ...
  for (const income of incomeRows) {
    ...
    for (const current of targetRows) {
      ...
      const next = {
        ...
        order_amount: Number(current.gross_product || 0)
          ? Math.max(Number(current.gross_product || 0) - Number(current.seller_discount || 0), 0)
...
async function computeSummary(filters = { preset: "all", month: "", store: "all" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    const sellerDiscountTotal = orderRows.reduce((sum, row) => sum + Math.abs(Number(row.seller_discount || 0)), 0);
    const productNetTotal = Math.max(grossSum - sellerDiscountTotal, 0);
    const lineOrderTotal = orderRows.reduce((sum, row) => sum + Math.abs(Number(row.order_amount || 0)), 0);
    const maxOrderAmount = Math.max(...orderRows.map(row => Math.abs(Number(row.order_amount || 0))), 0);
    const orderTotal = grossSum ? productNetTotal : (maxOrderAmount || lineOrderTotal || grossSum);
    const settlementTotal = Math.max(...orderRows.map(row => {
      if (String(row.source || "").toLowerCase() === "settlement" && !hasIncomeSettlement(row)) return 0;
      return Math.abs(Number(row.settlement_received || 0));
    }), 0);
    const refundTotal = Math.max(...orderRows.map(row => Math.abs(Number(row.refund_amount || 0))), 0);
    const rawPlatformFeeTotal = orderRows.reduce((sum, row) => sum + Math.abs(Number(row.platform_fee || 0)), 0);
    ...
    const cancelled = orderRows.some(row => isCancelledStatus(row.status));
    const unpaid = orderRows.some(row => isUnpaidStatus(row.status));
...
function buildAssistant(totals, margin, topSku, weakSku, missingCost, dailyList) {
  ...
  return {
    ...
    accounting: {
      pendapatan: omzet,
      omzetKotor: Number(totals.gross || 0),
      diskonSeller: Number(totals.sellerDiscount || 0),
      omzetNet: omzet,
      settlementCair: Number(totals.settlement || 0),
      hppPacking: Number(totals.hpp || 0) + Number(totals.packing || 0),
      hpp: Number(totals.hpp || 0),
      packing: Number(totals.packing || 0),
      potonganPlatform: platformFee,
      biayaIklan: adSpend,
      refund,
      returCancel: Number(totals.cancelledAmount || 0),
