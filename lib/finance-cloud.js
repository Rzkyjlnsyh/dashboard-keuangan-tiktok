...
function mapIncomeRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    const transactionId = normalizeOrderId(field(row, "Order/adjustment ID", "Order adjustment ID"));
    const relatedOrderId = normalizeOrderId(field(row, "Related order ID"));
...
async function computeSummary(filters = { preset: "all", month: "", store: "all" }) {
  ...
  for (const [groupKey, orderRows] of groups.entries()) {
    ...
    const packingTotal = financialRows.reduce((sum, row) => sum + Number(row.quantity || 0) * Number(row.packing_per_unit || 0), 0);
    ...
    for (const row of financialRows) {
      ...
      const packing = qty * Number(row.packing_per_unit || 0);
