...
async function importIncomeRows({ storeName, filename, rows, run }) {
  ...
  for (const income of incomeRows) {
    ...
    const targetRows = existingGroup.length ? existingGroup : [{
      line_key: lineKey(storeFilter, income.orderId, income.settlement < 0 || income.adjustment < 0 ? "BIAYA_ADJUSTMENT" : "INCOME", income.transactionId),
      order_id: income.orderId,
      store_name: storeFilter,
      source: "income_statement",
      created_at: income.createdAt,
      updated_at: income.settledAt,
      status: income.type || "Income",
      sku: income.settlement < 0 || income.adjustment < 0 ? "BIAYA_ADJUSTMENT" : "INCOME",
      product_name: income.type || "Income statement",
      variation: income.transactionId,
      quantity: 0,
      unit_price: 0,
      gross_product: Math.abs(income.beforeDiscount || income.afterDiscount || income.totalRevenue || 0),
      seller_discount: income.sellerDiscount,
      platform_discount: 0,
      platform_fee: 0,
      refund_amount: income.refund,
      order_amount: Math.abs(income.afterDiscount || income.totalRevenue || 0),
      settlement_received: Math.max(0, income.settlement),
      payment_method: "",
      tracking_id: "",
    ...
    const grossBase = targetRows.reduce((sum, row) => sum + Math.abs(Number(row.gross_product || 0)), 0) ||
      ...
      targetRows.length ||
      1;
    const incomeFee = income.totalFees || (income.settlement < 0 ? Math.abs(income.settlement) : 0) || (income.adjustment < 0 ? Math.abs(income.adjustment) : 0);
    const orderAmount = Math.abs(income.afterDiscount || income.totalRevenue || income.beforeDiscount || 0);
    const status = columnToken(income.type) === "order" && income.settledAt ? "Selesai" : income.type || "Income";
    ...
    for (const current of targetRows) {
      ...
      const next = {
        ...
        status,
        gross_product: current.gross_product || Math.abs(income.beforeDiscount || income.afterDiscount || income.totalRevenue || 0) * share,
        seller_discount: current.seller_discount || income.sellerDiscount * share,
        ...
        order_amount: orderAmount || Number(current.order_amount || 0),
        settlement_received: Math.max(Number(current.settlement_received || 0), Math.max(0, income.settlement)),
        last_seen_file: filename || "upload",
        last_seen_at: nowIso(),
      };
      const changes = auditChanges(existingGroup.length ? current : null, next);
      if (!existingGroup.length) inserted += 1;
  ...
  await finishImportRun(run && run.id, {
    ...
    message: "Income statement TikTok diperbarui",
  });
  return { ok: true, kind: "income", rows: rows.length, inserted, updated, unchanged, auditCount: auditEvents.length };
}
