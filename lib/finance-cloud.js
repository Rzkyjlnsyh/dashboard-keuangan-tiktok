...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "ads") {
    ...
    for (const row of cleanRows) {
      ...
      const fundType = String(field(row, "Fund type", "Account Type") || "").trim();
      const desc = String(field(row, "Description") || "").toLowerCase();
      if (type !== "General" || subtype !== "Add balance" || status !== "Success" || fundType !== "Cash") continue;
      if (desc.includes("gmv pay")) continue;
      const rawDate = field(row, "Transaction time") || "";
      const spendDate = rawDate instanceof Date && !isNaN(rawDate) ? rawDate.toISOString().slice(0,10) : (typeof rawDate === 'string' && rawDate.includes('T') ? rawDate.slice(0,10) : String(rawDate).replace(/\//g,'-').slice(0,10));
      const amount = Math.abs(rupiah(field(row, "Amount") || 0));
      if (!amount) continue;
      adRows.push({
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all", mode: "accrual" }) {
  ...
  // === OVERRIDE settlement/platform/penggantian/iklanGMV/refund with income_raw values ===
  // For SETTLEMENT mode only: use income_raw directly (cash basis)
  // For ACCRUAL mode: keep per-group computation (includes estimated piutang)
  if (incomeRawRows.length > 0 && calcMode === "settlement") {
    totals.settlement = Math.round(incomeSettlementCair);
    totals.platformFee = Math.round(incomePotonganPlatform);
    totals.platformFeeFinal = Math.round(incomePotonganPlatform);
    totals.platformFeeEstimated = 0;
    totals.adjustmentAmount = Math.round(incomePenggantian);
    totals.adSpendSettlement = Math.round(incomeIklanGMV);
    totals.settlementAdSpend = Math.round(incomeIklanGMV);
    totals.refund = Math.round(incomeRefundValid);
    // Book variants
    totals.bookSettlement = Math.round(incomeSettlementCair);
    totals.bookPlatformFee = Math.round(incomePotonganPlatform);
    totals.bookPlatformFeeFinal = Math.round(incomePotonganPlatform);
    totals.bookPlatformFeeEstimated = 0;
    totals.bookRefund = Math.round(incomeRefundValid);
    // Held = omzet net - potongan platform - settlement cair
    const omzetNet = totals.omzet || 0;
    const held = Math.max(0, omzetNet - Math.round(incomePotonganPlatform) - Math.round(incomeSettlementCair));
    totals.held = held;
    totals.bookHeld = held;
  } else if (incomeRawRows.length > 0) {
    // ACCRUAL mode: only override settlement/adSpendSettlement/refund from income_raw
    // Platform fees use per-group estimation (real + estimated piutang)
    totals.adSpendSettlement = Math.round(incomeIklanGMV);
    totals.settlementAdSpend = Math.round(incomeIklanGMV);
    totals.adjustmentAmount = Math.round(incomePenggantian);
    totals.refund = Math.round(incomeRefundValid);
