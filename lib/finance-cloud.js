...
async function upsertAdSpendRows(rows) {
  ...
  for (const row of rows) {
    const store = normalizeStore(row.store_name || DEFAULT_STORE);
    const spendDate = String(row.spend_date || todayIso()).slice(0, 10);
    const channel = String(row.channel || "TikTok Ads").trim() || "TikTok Ads";
    const campaign = String(row.campaign || "").trim();
    const existing = await fetchAll(
      "finance_ad_spend",
      `select=id&store_name=eq.${encodeURIComponent(store)}&spend_date=eq.${encodeURIComponent(spendDate)}&channel=eq.${encodeURIComponent(channel)}&campaign=eq.${encodeURIComponent(campaign)}&limit=1`
    );
    const prepared = {
      ...row,
      store_name: store,
    ...
    if (existing[0] && existing[0].id) {
      const patched = await supabaseRequest(`finance_ad_spend?id=eq.${encodeURIComponent(existing[0].id)}`, {
        method: "PATCH",
        headers: { Prefer: "return=representation" },
        body: JSON.stringify(prepared),
      });
      if (Array.isArray(patched)) saved.push(...patched);
    } else {
      const inserted = await insertRows("finance_ad_spend", [{ created_at: nowIso(), ...prepared }]);
      saved.push(...inserted);
    }
  }
  return saved;
