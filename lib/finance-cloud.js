...
function supabaseSetupMessage() {
  return "Supabase belum tersambung. Isi SUPABASE_URL dan SUPABASE_SERVICE_ROLE_KEY di Vercel, lalu jalankan SQL di supabase/schema.sql.";
}
...
function detectKind(rows, requestedKind) {
  ...
  const first = rows[0] || {};
  const cols = new Set(Object.keys(first).map(cleanCol));
  if (cols.has("sku") && cols.has("hppPerUnit") && cols.has("packingPerUnit")) return "sku";
  if (cols.has("Order ID") && cols.has("Seller SKU")) return "settlement";
  if (cols.has("Nomor Pesanan (di Marketplace)") && cols.has("SKU Marketplace")) return "orders";
  return "unknown";
}
...
async function importRows({ storeName, kind, filename, rows }) {
  ...
  if (detected === "unknown") throw new Error("Format file belum dikenal. Upload order Desty, pencairan TikTok, atau template SKU.");
