...
function buildAssistant(totals, margin, topSku, weakSku, missingCost, dailyList) {
  ...
  const accountingOmzet = hasBook ? Number(totals.bookOmzet || 0) : omzet;
...
function telegramMessage(summary) {
  ...
  return [
    ...
    `Omzet net akuntansi: ${money(accountingOmzet)}`,
    `Settlement cair: ${money(hasBook ? t.bookSettlement : t.settlement)}`,
    `Dana tertahan estimasi: ${money(t.held)}`,
    `Profit akuntansi: ${money(accountingProfit)} (${Number(accountingMargin || 0).toFixed(1)}%)`,
