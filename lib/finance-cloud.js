...
function isAdIncomeType(type) {
  ...
  return token.includes("ads") || token.includes("iklan") || token.includes("gmvmaxad") || token.includes("tiktokads");
}
...
function buildFilters(query = {}) {
  const preset = ["all", "last7", "last14", "thisMonth", "month"].includes(query.preset) ? query.preset : "thisMonth";
...
function mapIncomeRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    ...
    const adFee = Math.abs(rupiah(field(row, "GMV Max ad fee", "GMV Max Ad Fee", "Biaya iklan GMV Max", "Biaya Iklan GMV Max", "TikTok Ads fee", "Biaya TikTok Ads")));
...
async function importIncomeRows({ storeName, filename, rows, run }) {
  ...
  for (const income of incomeRows) {
    ...
    if (adAmount > 0) {
      ...
      const campaign = `${filename || "income"}:${income.transactionId || income.orderId}:ads`;
      ...
      adSpendByKey.set(key, {
        ...
        campaign,
        note: `Terdeteksi dari income statement: ${income.type || "ads"}`,
        created_at: nowIso(),
        updated_at: nowIso(),
      });
    }
    ...
    const incomeFee =
      Math.max(Number(income.totalFees || 0) - Number(income.adFee || 0), 0) ||
  ...
  await upsertRows("finance_ad_spend", Array.from(adSpendByKey.values()), "finance_ad_spend_unique_row");
...
async function saveAdSpend(data) {
  ...
  const saved = await upsertRows("finance_ad_spend", [row], "finance_ad_spend_unique_row");
  return saved[0] || row;
}
...
async function computeSummary(filters = { preset: "thisMonth", month: "", store: "all" }) {
  ...
  const explicitAdSpend = adRows.reduce((sum, row) => sum + Number(row.amount || 0), 0);
  const treatSettlementGapAsAd = explicitAdSpend <= 0;

  for (const [groupKey, orderRows] of groups.entries()) {
    const basisRows = orderRows.filter(row => isBookSource(row));
    let financialRows = basisRows.length ? basisRows : orderRows;
    ...
    let finalPlatformFeeTotal = hasIncome ? rawPlatformFeeTotal : 0;
    ...
    const packageCount = Math.max(new Set(financialRows.map(row => String(row.tracking_id || "").trim()).filter(Boolean)).size, 1);
    const packingTotal = Math.max(...financialRows.map(row => Number(row.packing_per_unit || 0)), 0) * packageCount;
    const returned = financialRows.some(row => isReturnStatus(row.status));
    const cancelOnly = financialRows.some(row => isCancelStatus(row.status)) && !returned;
    ...
    const settlementGap = !excluded && hasIncome && settlementTotal
      ? Math.max(orderTotal - platformFeeTotal - settlementTotal, 0)
      : 0;
    if (settlementGap > 0 && !treatSettlementGapAsAd) {
      finalPlatformFeeTotal += settlementGap;
      platformFeeTotal += settlementGap;
    }
    ...
    const bucket = operationBucket(first.status, { returned, cancelOnly, unpaid, isFinal, isEstimated });
    const ageDays = dayAge(createdDay, today);
    const late = ["processing", "waiting_ship"].includes(bucket) && ageDays > 0;
    const op = operationSummary.get(bucket) || {
      bucket,
      label: OPERATION_LABELS[bucket] || "Lainnya",
      orders: new Set(),
      packages: 0,
      late: 0,
    };
    op.orders.add(orderId);
    op.packages += packageCount;
    if (late) op.late += packageCount;
    operationSummary.set(bucket, op);
    const trackingIds = Array.from(new Set(financialRows.map(row => String(row.tracking_id || "").trim()).filter(Boolean)));
    operationDetails.push({
      ...
      label: OPERATION_LABELS[bucket] || "Lainnya",
      packageCount,
      itemQty: financialRows.reduce((sum, row) => sum + Number(row.quantity || 0), 0),
      skuCount: new Set(financialRows.map(row => row.sku).filter(Boolean)).size,
    ...
    if (excluded) {
      ...
      if (settlementGap > 0 && treatSettlementGapAsAd) {
        totals.adSpend += settlementGap;
        totals.settlementAdSpend += settlementGap;
        daily.get(createdDay).profit -= settlementGap;
        stores.get(store).profit -= settlementGap;
      }
    ...
    for (const row of financialRows) {
      ...
      const packing = costBurned ? packingTotal * share : 0;
  ...
  if (totals.omzet) {
    const datedAdSpend = Math.max(totals.adSpend - totals.settlementAdSpend, 0);
    totals.finalAdSpend = totals.settlementAdSpend + datedAdSpend * (totals.finalOmzet / totals.omzet);
