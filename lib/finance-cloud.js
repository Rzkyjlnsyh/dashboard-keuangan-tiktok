...
function mapIncomeRows(rows, storeName, filename) {
  ...
  return rows.map(row => {
    ...
    const adFee =
      Math.abs(rupiah(field(row, "GMV Max ad fee", "GMV Max Ad Fee", "Biaya iklan GMV Max", "Biaya Iklan GMV Max", "TikTok Ads fee", "Biaya TikTok Ads"))) +
      Math.abs(rupiah(field(row, "Affiliate Shop Ads commission", "Affiliate Partner shop ads commission", "Campaign resource fee")));
