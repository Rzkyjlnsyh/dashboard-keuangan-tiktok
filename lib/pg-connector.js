...
async function upsertAdSpendRows(rows) {
  ...
  if (mode === 'postgres' && pool) {
    ...
    for (const row of rows) {
      try {
        const ch = row.channel || '', ca = row.campaign || '';
        const ex = await pool.query(
          'SELECT 1 FROM finance_ad_spend WHERE store_name=$1 AND spend_date=$2 AND COALESCE(channel,\'\')=COALESCE($3,\'\') AND COALESCE(campaign,\'\')=COALESCE($4,\'\')',
          [row.store_name, row.spend_date, ch, ca]
        );
        if (ex.rows.length > 0) continue;
        await pool.query(
          'INSERT INTO finance_ad_spend(store_name,spend_date,amount,channel,campaign,note,created_at,updated_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8)',
          [row.store_name, row.spend_date, row.amount, ch, ca, row.note||'', row.created_at, row.updated_at]
        );
        saved.push(row);
      } catch(e) { console.error('[db] ad insert err:', e.message.substring(0,60)); }
    }
    return saved;
  }
...
async function initSchema() {
  const ddl = `
    CREATE TABLE IF NOT EXISTS finance_order_lines(line_key TEXT PRIMARY KEY,order_id TEXT,store_name TEXT,source TEXT,created_at TEXT,updated_at TEXT,status TEXT,sku TEXT,product_name TEXT,variation TEXT,quantity REAL,unit_price REAL,gross_product REAL,seller_discount REAL,platform_discount REAL,platform_fee REAL,refund_amount REAL,order_amount REAL,settlement_received REAL,payment_method TEXT,tracking_id TEXT,package_id TEXT,cancel_reason TEXT,shipped_time TEXT,adjustment_amount REAL,last_seen_file TEXT,last_seen_at TEXT);
