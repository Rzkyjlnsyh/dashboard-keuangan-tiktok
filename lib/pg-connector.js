...
async function upsertRows(table, rows, conflictKey) {
  ...
  if(mode==='postgres' && pool) {
    ...
    for(const row of rows) {
      const cols=Object.keys(row), keys=Array.isArray(conflictKey)?conflictKey:[conflictKey];
      const allCols=cols.map(c=>`"${c}"`).join(',');
      const vals=cols.map(c=>row[c]);
      const ph=vals.map((_,i)=>'$'+(i+1)).join(',');
      const setClause=cols.filter(c=>!keys.includes(c)).map(c=>`"${c}"=EXCLUDED."${c}"`).join(',');
      const conflictCols=keys.map(k=>`"${k}"`).join(',');
      try {
        await pool.query(`INSERT INTO "${table}" (${allCols}) VALUES (${ph}) ON CONFLICT (${conflictCols}) DO UPDATE SET ${setClause}`, vals);
        saved.push(row);
      } catch(e) { console.error('[db] upsert error:',e.message.substring(0,80)); }
    }
    return saved;
  }
  const d=getDb(); if(!d) return [];
