/**
 * Database Connector — Dual Mode
 * - Local: SQLite (better-sqlite3)
 * - Production: PostgreSQL (pg) via Supabase / DATABASE_URL
 */
let db=null, pool=null, mode='sqlite';

// Detect environment
if (process.env.DATABASE_URL) {
  mode = 'postgres';
  try {
    const { Pool } = require('pg');
    pool = new Pool({
      connectionString: process.env.DATABASE_URL,
      ssl: { rejectUnauthorized: false }
    });
    console.log('[db] PostgreSQL mode');
  } catch(e) {
    console.error('[db] PostgreSQL init failed, fallback to SQLite:', e.message);
    mode = 'sqlite';
  }
}

if (mode === 'sqlite') {
  try {
    const path=require('path');
    const Database=require('better-sqlite3');
    const DB_PATH=path.join(__dirname,'..','data','finance.db');
    const fs=require('fs');
    const dir=path.dirname(DB_PATH);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, {recursive:true});
    db=new Database(DB_PATH);
    db.pragma('journal_mode=WAL');
    console.log('[db] SQLite mode:', DB_PATH);
  } catch(e) {
    console.error('[db] SQLite not available ('+e.message+'), using PostgreSQL mode');
    mode = 'postgres';
    // Try PostgreSQL if DATABASE_URL not set yet
    if (!pool && process.env.DATABASE_URL) {
      try {
        const { Pool } = require('pg');
        pool = new Pool({
          connectionString: process.env.DATABASE_URL,
          ssl: { rejectUnauthorized: false }
        });
        console.log('[db] PostgreSQL mode (fallback)');
      } catch(e2) {
        console.error('[db] PostgreSQL also failed:', e2.message);
      }
    }
  }
}

function getDb() {
  if (mode === 'postgres') return null; // PostgreSQL mode, use pool directly
  return db;
}

function pgConfigured() { return true; }
function pgSetupMessage() { return ''; }

async function pgQuery(sql, params=[]) {
  if (mode === 'postgres') {
    if (!pool) return { rows: [] };
    const client = await pool.connect().catch(() => null);
    if (!client) return { rows: [] };
    try {
      const result = await client.query(sql, params);
      return { rows: result.rows || [], changes: result.rowCount || 0 };
    } catch(e) {
      console.error('[db] PG query error:', e.message.substring(0, 80));
      return { rows: [] };
    } finally {
      try { client.release(); } catch(e) {}
    }
  }
  // SQLite mode
  const d = getDb();
  if (!d) return { rows: [] };
  const upper = sql.trim().toUpperCase();
  // Convert PostgreSQL-style placeholders ($1,$2) to SQLite (?)
  let sqliteSql = sql;
  if (sql.includes('$')) {
    // Replace $1, $2, etc with ?
    sqliteSql = sql.replace(/\$\d+/g, '?');
    // Remove PostgreSQL-specific syntax
    sqliteSql = sqliteSql.replace(/::date\s*\+\s*interval\s*'\d+\s*day'/gi, '');
    sqliteSql = sqliteSql.replace(/::date/gi, '');
    sqliteSql = sqliteSql.replace(/::text/gi, '');
    sqliteSql = sqliteSql.replace(/\(\s*\$2::date\+interval'1 day'\)::text/gi, '?');
  }
  try {
    if (upper.startsWith('SELECT')) {
      return { rows: d.prepare(sqliteSql).all(...params) };
    }
    if (upper.startsWith('INSERT') || upper.startsWith('UPDATE') || upper.startsWith('DELETE') || upper.startsWith('CREATE')) {
      const stmt = d.prepare(sqliteSql);
      const result = stmt.run(...params);
      if (upper.includes('RETURNING')) {
        return { rows: [], changes: result.changes };
      }
      return { rows: [], changes: result.changes };
    }
    return { rows: [] };
  } catch(e) {
    console.error('[db] SQLite error:', e.message.substring(0, 100));
    return { rows: [] };
  }
}

async function fetchAll(table, queryStr='') {
  if (mode === 'postgres' && pool) {
    // Build simple WHERE from queryStr
    let sql = `SELECT * FROM "${table}"`;
    let where = [];
    let order = '';
    let limit = 4000;
    const parts = (queryStr||'').split('&').filter(Boolean);
    for (const part of parts) {
      const [k, v] = part.split('='); if (!k || !v) continue;
      if (k === 'select') {
        if (v !== '*') sql = sql.replace('*', v.split(',').map(c => `"${c.trim()}"`).join(', '));
      } else if (k === 'order') {
        const [col, dir] = v.split('.');
        order = `ORDER BY "${col}" ${dir === 'desc' ? 'DESC' : 'ASC'}`;
      } else if (k === 'limit') limit = parseInt(v) || 4000;
      else if (k === 'and') {
        // Handle complex AND conditions
        const inner = v.startsWith('(') ? v.slice(1, -1) : v;
        const andParts = inner.split(',');
        for (const ap of andParts) {
          const dot = ap.indexOf('.'); if (dot < 0) continue;
          const col = ap.slice(0, dot);
          const val = ap.slice(dot + 1);
          const opDot = val.indexOf('.');
          if (opDot < 0) { where.push(`"${col}" = '${val.replace(/'/g, "''")}'`); continue; }
          const op = val.slice(0, opDot);
          const raw = decodeURIComponent(val.slice(opDot + 1));
          if (op === 'eq') where.push(`"${col}" = '${raw.replace(/'/g, "''")}'`);
          else if (op === 'gte') where.push(`"${col}" >= '${raw.replace(/'/g, "''")}'`);
          else if (op === 'lte') where.push(`"${col}" <= '${raw.replace(/'/g, "''")}'`);
          else if (op === 'like') where.push(`"${col}" LIKE '${raw.replace(/'/g, "''")}'`);
        }
      } else {
        const dot = v.indexOf('.');
        if (dot < 0) { where.push(`"${k}" = '${v.replace(/'/g, "''")}'`); continue; }
        const op = v.slice(0, dot);
        const raw = decodeURIComponent(v.slice(dot + 1));
        if (op === 'eq') where.push(`"${k}" = '${raw.replace(/'/g, "''")}'`);
        else if (op === 'neq') where.push(`"${k}" != '${raw.replace(/'/g, "''")}'`);
      }
    }
    if (where.length) sql += ' WHERE ' + where.join(' AND ');
    if (order) sql += ' ' + order;
    sql += ' LIMIT ' + limit;
    try {
      const result = await pool.query(sql);
      return result.rows;
    } catch(e) {
      console.error('[db] PG fetchAll error:', e.message.substring(0, 100));
      return [];
    }
  }
  // SQLite mode
  const d = getDb();
  if (!d) return [];
  let sql = `SELECT * FROM "${table}"`;
  let where = [];
  let order = '';
  let limit = 4000;
  const parts = (queryStr||'').split('&').filter(Boolean);
  for (const part of parts) {
    const [k, v] = part.split('='); if (!k || !v) continue;
    if (k === 'select') {
      if (v !== '*') sql = sql.replace('*', v.split(',').map(c => `"${c.trim()}"`).join(', '));
    } else if (k === 'order') {
      const [col, dir] = v.split('.');
      order = `ORDER BY "${col}" ${dir === 'desc' ? 'DESC' : 'ASC'}`;
    } else if (k === 'limit') limit = parseInt(v) || 4000;
    else if (k === 'and') {
      const inner = v.startsWith('(') ? v.slice(1, -1) : v;
      const andParts = inner.split(',');
      for (const ap of andParts) {
        const dot = ap.indexOf('.'); if (dot < 0) continue;
        const col = ap.slice(0, dot);
        const val = ap.slice(dot + 1);
        const opDot = val.indexOf('.');
        if (opDot < 0) { where.push(`"${col}"='${val.replace(/'/g, "''")}'`); continue; }
        const op = val.slice(0, opDot);
        const raw = decodeURIComponent(val.slice(opDot + 1));
        if (op === 'eq') where.push(`"${col}"='${raw.replace(/'/g, "''")}'`);
        if (op === 'gte') where.push(`"${col}">='${raw.replace(/'/g, "''")}'`);
        if (op === 'lte') where.push(`"${col}"<='${raw.replace(/'/g, "''")}'`);
        if (op === 'like') where.push(`"${col}" LIKE '${raw.replace(/'/g, "''")}'`);
      }
    } else {
      const dot = v.indexOf('.'); if (dot < 0) { where.push(`"${k}"='${v.replace(/'/g, "''")}'`); continue; }
      const op = v.slice(0, dot);
      const raw = decodeURIComponent(v.slice(dot + 1));
      if (op === 'eq') where.push(`"${k}"='${raw.replace(/'/g, "''")}'`);
      if (op === 'neq') where.push(`"${k}"!='${raw.replace(/'/g, "''")}'`);
    }
  }
  if (where.length) sql += ' WHERE ' + where.join(' AND ');
  if (order) sql += ' ' + order;
  sql += ' LIMIT ' + limit;
  try {
    return d.prepare(sql).all();
  } catch(e) {
    console.error('[db] SQLite fetchAll error:', e.message.substring(0, 100));
    return [];
  }
}

async function upsertRows(table, rows, conflictKey) {
  if (!rows || !rows.length) return [];
  if (mode === 'postgres' && pool) {
    const saved = [];
    for (const row of rows) {
      const cols = Object.keys(row);
      const keys = Array.isArray(conflictKey) ? conflictKey : [conflictKey];
      const conflictCols = keys.map(k => `"${k}"`).join(',');
      const setCols = cols.filter(c => !keys.includes(c));
      const setClause = setCols.map(c => `"${c}" = EXCLUDED."${c}"`).join(',');
      const allCols = cols.map(c => `"${c}"`).join(',');
      const values = cols.map(c => row[c]);
      const placeholders = values.map((_, i) => '$' + (i + 1)).join(',');
      const sql = `INSERT INTO "${table}" (${allCols}) VALUES (${placeholders}) ON CONFLICT (${conflictCols}) DO UPDATE SET ${setClause}`;
      try {
        await pool.query(sql, values);
        saved.push(row);
      } catch(e) {
        console.error('[db] PG upsert error:', e.message.substring(0, 100));
      }
    }
    return saved;
  }
  // SQLite mode
  const d = getDb();
  if (!d) return [];
  const keys = Array.isArray(conflictKey) ? conflictKey : [conflictKey];
  const saved = [];
  for (const row of rows) {
    const cols = Object.keys(row);
    const existing = d.prepare(`SELECT 1 FROM "${table}" WHERE ${keys.map(k => `"${k}"=?`).join(' AND ')}`).get(...keys.map(k => row[k]));
    if (existing) {
      const sets = cols.filter(c => !keys.includes(c)).map(c => `"${c}"=?`).join(',');
      d.prepare(`UPDATE "${table}" SET ${sets} WHERE ${keys.map(k => `"${k}"=?`).join(' AND ')}`).run(...cols.filter(c => !keys.includes(c)).map(c => row[c]), ...keys.map(k => row[k]));
    } else {
      const cnames = cols.map(c => `"${c}"`).join(',');
      const ph = cols.map(() => '?').join(',');
      d.prepare(`INSERT INTO "${table}" (${cnames}) VALUES (${ph})`).run(...cols.map(c => row[c]));
    }
    saved.push(row);
  }
  return saved;
}

async function insertRows(table, rows) {
  if (!rows || !rows.length) return [];
  if (mode === 'postgres' && pool) {
    for (const row of rows) {
      const cols = Object.keys(row);
      const values = cols.map(c => row[c]);
      const placeholders = values.map((_, i) => '$' + (i + 1)).join(',');
      const sql = `INSERT INTO "${table}" (${cols.map(c => `"${c}"`).join(',')}) VALUES (${placeholders})`;
      try { await pool.query(sql, values); } catch(e) {}
    }
    return rows;
  }
  const d = getDb();
  if (!d) return [];
  for (const row of rows) {
    const cols = Object.keys(row);
    try {
      d.prepare(`INSERT INTO "${table}" (${cols.map(c => `"${c}"`).join(',')}) VALUES (${cols.map(() => '?').join(',')})`).run(...cols.map(c => row[c]));
    } catch(e) {}
  }
  return rows;
}

async function upsertAdSpendRows(rows) {
  return upsertRows('finance_ad_spend', rows, ['store_name', 'spend_date', 'channel', 'campaign']);
}

async function initSchema() {
  if (mode === 'postgres' && pool) {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS finance_order_lines(line_key TEXT PRIMARY KEY,order_id TEXT,store_name TEXT DEFAULT 'ventura',source TEXT,created_at TEXT,updated_at TEXT,status TEXT,sku TEXT,product_name TEXT,variation TEXT,quantity REAL DEFAULT 0,unit_price REAL DEFAULT 0,gross_product REAL DEFAULT 0,seller_discount REAL DEFAULT 0,platform_discount REAL DEFAULT 0,platform_fee REAL DEFAULT 0,refund_amount REAL DEFAULT 0,order_amount REAL DEFAULT 0,settlement_received REAL DEFAULT 0,payment_method TEXT,tracking_id TEXT,package_id TEXT,cancel_reason TEXT,shipped_time TEXT,adjustment_amount REAL DEFAULT 0,last_seen_file TEXT,last_seen_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_sku_costs(sku_key TEXT PRIMARY KEY,store_name TEXT DEFAULT 'global',sku TEXT,product_name TEXT,hpp_per_unit REAL DEFAULT 0,packing_per_unit REAL DEFAULT 0,updated_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_ad_spend(id SERIAL PRIMARY KEY,store_name TEXT DEFAULT 'ventura',spend_date TEXT,amount REAL DEFAULT 0,channel TEXT,campaign TEXT,note TEXT,created_at TEXT,updated_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_import_runs(id SERIAL PRIMARY KEY,filename TEXT,kind TEXT,store_name TEXT,rows_seen INTEGER DEFAULT 0,inserted INTEGER DEFAULT 0,updated INTEGER DEFAULT 0,unchanged INTEGER DEFAULT 0,audit_count INTEGER DEFAULT 0,message TEXT,created_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_audit_events(id TEXT PRIMARY KEY,run_id INTEGER,filename TEXT,kind TEXT,store_name TEXT,order_id TEXT,sku TEXT,field_name TEXT,old_value TEXT,new_value TEXT,change_type TEXT,created_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_config(key TEXT PRIMARY KEY,value TEXT DEFAULT '{}',updated_at TEXT);
      CREATE TABLE IF NOT EXISTS finance_income_raw(id SERIAL PRIMARY KEY,store_name TEXT,transaction_type TEXT,order_id TEXT,order_created_time TEXT,settlement_amount REAL DEFAULT 0,total_fees REAL DEFAULT 0,refund_amount REAL DEFAULT 0,adjustment_amount REAL DEFAULT 0,imported_at TEXT);
    `);
    // Create indexes
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_ol_store ON finance_order_lines(store_name)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_ol_created ON finance_order_lines(created_at)`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_ir_month ON finance_income_raw(store_name,order_created_time)`);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_ad_unique ON finance_ad_spend(store_name,spend_date,COALESCE(channel,''),COALESCE(campaign,''))`);
    await pool.query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_income_unique ON finance_income_raw(store_name,order_id,transaction_type)`);
    console.log('[db] PostgreSQL schema initialized');
    return;
  }
  // SQLite mode
  const d = getDb();
  if (!d) return;
  d.exec(`
    CREATE TABLE IF NOT EXISTS finance_order_lines(line_key TEXT PRIMARY KEY,order_id TEXT,store_name TEXT DEFAULT 'ventura',source TEXT,created_at TEXT,updated_at TEXT,status TEXT,sku TEXT,product_name TEXT,variation TEXT,quantity REAL DEFAULT 0,unit_price REAL DEFAULT 0,gross_product REAL DEFAULT 0,seller_discount REAL DEFAULT 0,platform_discount REAL DEFAULT 0,platform_fee REAL DEFAULT 0,refund_amount REAL DEFAULT 0,order_amount REAL DEFAULT 0,settlement_received REAL DEFAULT 0,payment_method TEXT,tracking_id TEXT,package_id TEXT,cancel_reason TEXT,shipped_time TEXT,adjustment_amount REAL DEFAULT 0,last_seen_file TEXT,last_seen_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_sku_costs(sku_key TEXT PRIMARY KEY,store_name TEXT DEFAULT 'global',sku TEXT,product_name TEXT,hpp_per_unit REAL DEFAULT 0,packing_per_unit REAL DEFAULT 0,updated_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_ad_spend(id INTEGER PRIMARY KEY AUTOINCREMENT,store_name TEXT DEFAULT 'ventura',spend_date TEXT,amount REAL DEFAULT 0,channel TEXT,campaign TEXT,note TEXT,created_at TEXT,updated_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_import_runs(id INTEGER PRIMARY KEY AUTOINCREMENT,filename TEXT,kind TEXT,store_name TEXT,rows_seen INTEGER DEFAULT 0,inserted INTEGER DEFAULT 0,updated INTEGER DEFAULT 0,unchanged INTEGER DEFAULT 0,audit_count INTEGER DEFAULT 0,message TEXT,created_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_audit_events(id INTEGER PRIMARY KEY AUTOINCREMENT,run_id INTEGER,filename TEXT,kind TEXT,store_name TEXT,order_id TEXT,sku TEXT,field_name TEXT,old_value TEXT,new_value TEXT,change_type TEXT,created_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_config(key TEXT PRIMARY KEY,value TEXT DEFAULT '{}',updated_at TEXT);
    CREATE TABLE IF NOT EXISTS finance_income_raw(id INTEGER PRIMARY KEY AUTOINCREMENT,store_name TEXT,transaction_type TEXT,order_id TEXT,order_created_time TEXT,settlement_amount REAL DEFAULT 0,total_fees REAL DEFAULT 0,refund_amount REAL DEFAULT 0,adjustment_amount REAL DEFAULT 0,imported_at TEXT);
    CREATE INDEX IF NOT EXISTS idx_ol_store ON finance_order_lines(store_name);
    CREATE INDEX IF NOT EXISTS idx_ol_created ON finance_order_lines(created_at);
    CREATE INDEX IF NOT EXISTS idx_ir_month ON finance_income_raw(store_name,order_created_time);
    CREATE UNIQUE INDEX IF NOT EXISTS idx_ad_unique ON finance_ad_spend(store_name,spend_date,COALESCE(channel,''),COALESCE(campaign,''));
    CREATE UNIQUE INDEX IF NOT EXISTS idx_income_unique ON finance_income_raw(store_name,order_id,transaction_type);
  `);
  console.log('[db] SQLite schema initialized');
}

async function cacheGet(key) { return null; }
async function cacheSet(key, data, ttl) {}
async function cacheDelete(pattern) {}
async function cacheDeleteByTable(table) {}

module.exports = {
  pgConfigured, pgSetupMessage, pgQuery, fetchAll, upsertRows, insertRows,
  upsertAdSpendRows, initSchema, cacheGet, cacheSet, cacheDelete, cacheDeleteByTable, getDb
};
