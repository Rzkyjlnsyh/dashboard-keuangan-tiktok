...
function getPool() {
  ...
  const env = pgEnv();
  pool = env.url
    ? new Pool({ connectionString: env.url, connectionTimeoutMillis: 10000, max: 5, idleTimeoutMillis: 15000 })
...
function parseQueryParams(qs) {
  ...
  for (const part of qs.split('&')) {
    ...
    if (k === 'select') {
      ...
      // Handle format: and=(field.op.val,field2.op.val)
      const inner = v.startsWith('(') && v.endsWith(')') ? v.slice(1, -1) : v;
      r.where.push(parseAndGroup(inner));
...
async function fetchAll(table, queryParams = '') {
  ...
  const pageSize = params.limit || 4000;
  const allRows = [];
  let offset = 0;
  while (true) {
...
async function cacheGet(key) {
  ...
  try {
    const r = await pgQuery('SELECT data FROM "finance_data_cache" WHERE cache_key=$1 AND (expires_at IS NULL OR expires_at > to_char(NOW(),'YYYY-MM-DD HH24:MI:SS'))', [key]);
...
async function cacheSet(key, data, ttlSeconds = 300) {
  ...
  try {
    const now = new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Jakarta' });
    const exp = new Date(Date.now() + ttlSeconds * 1000).toLocaleString('sv-SE', { timeZone: 'Asia/Jakarta' });
    await pgQuery('INSERT INTO "finance_data_cache"(cache_key,data,created_at,expires_at) VALUES($1,$2::jsonb,$3,$4) ON CONFLICT(cache_key) DO UPDATE SET data=$2::jsonb,created_at=$3,expires_at=$4', [key, JSON.stringify(data), now, exp]);
  } catch(e) {}
}
...
async function cacheDelete(pattern) {
  if (!pgConfigured()) return;
  try {
    await pgQuery('DELETE FROM "finance_data_cache" WHERE cache_key LIKE $1', [pattern]);
  } catch(e) {}
}

async function cacheDeleteByTable(table) {
  await cacheDelete('summary:' + table + ':%');
  await cacheDelete('split:%');
}
...
async function initSchema() {
  ...
  const stmts = `
    ...
    CREATE TABLE IF NOT EXISTS "finance_data_cache" ( "cache_key" TEXT PRIMARY KEY, "data" JSONB NOT NULL DEFAULT '{}', "created_at" TEXT, "expires_at" TEXT );
